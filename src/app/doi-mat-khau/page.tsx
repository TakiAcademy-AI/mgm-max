import Link from "next/link";
import { redirect } from "next/navigation";
import { AlertCircle, KeyRound, Rocket, ShieldCheck } from "lucide-react";
import { MAT_KHAU_MAC_DINH, phaiDoiMatKhau, thanhVienHienTai } from "@/services/tai-khoan";
import { actDoiMatKhau } from "../tai-khoan/actions";

export const dynamic = "force-dynamic";

export default async function TrangDoiMatKhau(props: {
  searchParams: Promise<{ loi?: string; dau?: string }>;
}) {
  const { loi = "", dau = "" } = await props.searchParams;
  const tv = await thanhVienHienTai();
  if (!tv) redirect("/dang-nhap");

  const batBuoc = phaiDoiMatKhau(tv);   // còn dùng mật khẩu mặc định → bắt buộc đổi

  return (
    <main className="relative min-h-screen overflow-hidden bg-slate-50 px-4 py-14">
      <div className="pointer-events-none absolute -top-24 left-1/2 h-80 w-[36rem] -translate-x-1/2 rounded-full bg-gradient-to-r from-blue-200 via-violet-200 to-pink-200 opacity-60 blur-3xl" />
      <div className="relative mx-auto max-w-md">
        <Link href="/" className="mx-auto flex w-fit items-center gap-2.5">
          <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-blue-600 to-violet-600 text-white shadow-sm">
            <Rocket className="h-5 w-5" />
          </span>
          <span className="text-xl font-black tracking-tight text-slate-900">MGM <span className="text-blue-600">MAX</span></span>
        </Link>

        <div className="the mt-7 p-6 sm:p-8">
          <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-blue-100">
            <ShieldCheck className="h-6 w-6 text-blue-700" />
          </div>
          <h1 className="mt-4 text-2xl font-black text-slate-900">
            {batBuoc ? "Đặt mật khẩu mới cho tài khoản" : "Đổi mật khẩu"}
          </h1>
          <p className="mt-1.5 text-sm text-slate-500">
            {batBuoc ? (
              <>Chào {tv.ten || tv.email}! Bạn đang dùng <b>mật khẩu mặc định</b> — hãy đặt mật khẩu riêng để bảo vệ tài khoản trước khi tiếp tục.</>
            ) : (
              <>Đặt mật khẩu mới cho tài khoản <b>{tv.email}</b>. Mọi thiết bị đang đăng nhập sẽ phải đăng nhập lại.</>
            )}
          </p>

          {loi && (
            <div className="mt-5 flex items-start gap-2.5 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-medium text-red-700">
              <AlertCircle className="mt-0.5 h-4.5 w-4.5 shrink-0" /> {loi}
            </div>
          )}

          <form action={actDoiMatKhau} className="mt-6 space-y-4">
            {dau && <input type="hidden" name="dau" value="1" />}
            {batBuoc ? (
              <>
                <input type="hidden" name="mat_khau_cu" value={MAT_KHAU_MAC_DINH} />
                <div className="rounded-xl bg-slate-50 px-4 py-3 text-sm text-slate-500">
                  Mật khẩu hiện tại: <b className="font-mono text-slate-700">mật khẩu mặc định</b>
                </div>
              </>
            ) : (
              <div>
                <label className="nhan" htmlFor="mat_khau_cu">Mật khẩu hiện tại</label>
                <div className="relative">
                  <KeyRound className="pointer-events-none absolute left-3.5 top-1/2 h-4.5 w-4.5 -translate-y-1/2 text-slate-400" />
                  <input id="mat_khau_cu" name="mat_khau_cu" type="password" required autoComplete="current-password" className="o-nhap !pl-11" />
                </div>
              </div>
            )}
            <div>
              <label className="nhan" htmlFor="mat_khau_moi">Mật khẩu mới</label>
              <input id="mat_khau_moi" name="mat_khau_moi" type="password" required minLength={6} autoFocus={batBuoc}
                autoComplete="new-password" placeholder="Ít nhất 6 ký tự" className="o-nhap" />
            </div>
            <div>
              <label className="nhan" htmlFor="mat_khau_lai">Nhập lại mật khẩu mới</label>
              <input id="mat_khau_lai" name="mat_khau_lai" type="password" required minLength={6}
                autoComplete="new-password" placeholder="Gõ lại cho chắc" className="o-nhap" />
            </div>
            <button className="nut-chinh w-full !py-3"><ShieldCheck className="h-4.5 w-4.5" /> Lưu mật khẩu mới</button>
          </form>

          {!batBuoc && (
            <div className="mt-5 text-center">
              <Link href="/tai-khoan" className="text-sm font-semibold text-slate-500 hover:text-slate-700">← Quay lại tài khoản</Link>
            </div>
          )}
        </div>
      </div>
    </main>
  );
}
